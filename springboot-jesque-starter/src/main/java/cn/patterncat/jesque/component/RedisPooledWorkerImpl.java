...
/**
 * 没有使用lua script之前的redis连接池版本
 * 在2.1.0版本的WorkerImpl基础上修改
...
public class RedisPooledWorkerImpl implements Worker {
    ...
    protected long emptyQueueSleepTimeInMillis = 500L;
     ...
     */
    public RedisPooledWorkerImpl(final Config config, final Collection<String> queues, final JobFactory jobFactory,
                                 final Pool<Jedis> jedisPool, long emptyQueueSleepTimeInMillis) {
        if (config == null) {
        ...
        this.emptyQueueSleepTimeInMillis = emptyQueueSleepTimeInMillis;
    }
    ...
    public void run() {
        if (this.state.compareAndSet(NEW, RUNNING)) {
            try {
                ...
                PoolUtils.doWorkInPool(jedisPool, new PoolUtils.PoolWork<Jedis, Void>() {
                    @Override
                    public Void doWork(Jedis jedis) throws Exception {
                        ...
                        listenerDelegate.fireEvent(WORKER_START, RedisPooledWorkerImpl.this, null, null, null, null, null);
                        return null;
                    }
                });
                ...
                PoolUtils.doWorkInPoolNicely(jedisPool, new PoolUtils.PoolWork<Jedis, Void>() {
                    @Override
                    public Void doWork(Jedis jedis) throws Exception {
    ...
    public void setQueues(final Collection<String> queues) {
        ...
        if (queues == ALL_QUEUES) { // Using object equality on purpose
            ...
            this.queueNames.addAll(PoolUtils.doWorkInPoolNicely(this.jedisPool, new PoolUtils.PoolWork<Jedis, Set<String>>() {
                /**
                 * {@inheritDoc}
                 */
                @Override
    ...
    protected void poll() {
        ...
        while (RUNNING.equals(this.state.get())) {
            try {
                ...
                curQueue = this.queueNames.poll(getEmptyQueueSleepTimeInMillis(), TimeUnit.MILLISECONDS);
                if (curQueue != null) {
    ...
    protected String pop(final String curQueue) {
        ...
        payload = PoolUtils.doWorkInPoolNicely(jedisPool, new PoolUtils.PoolWork<Jedis, String>() {
            @Override
            public String doWork(Jedis jedis) throws Exception {
                ...
                if (JedisUtils.isDelayedQueue(jedis, key)) {
                    ...
                    result = lpoplpush(jedis,key, key(INFLIGHT, name, curQueue));
                }
                return result;
            }
        });
    ...
    public long getEmptyQueueSleepTimeInMillis() {
        return emptyQueueSleepTimeInMillis;
    }
