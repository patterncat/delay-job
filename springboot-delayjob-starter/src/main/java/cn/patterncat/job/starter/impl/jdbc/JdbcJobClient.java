package cn.patterncat.job.starter.impl.jdbc;

import cn.patterncat.job.starter.api.JobClient;
import cn.patterncat.job.starter.impl.jdbc.dao.PendingJobDao;
...
public class JdbcJobClient implements JobClient<Long,PendingJob> {
    ...
    public JobResp<PendingJob> newJob(PendingJob jobInfo) {
        try{
            ...
            LOGGER.error(e.getMessage(),e);
            return JobResp.<PendingJob>builder().throwable(e).success(false).build();
        }
    }

    @Override
    public JobResp claimJob(Long jobId, String instanceId) {
        return null;
    }
